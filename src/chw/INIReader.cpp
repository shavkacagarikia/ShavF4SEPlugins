...
void SetKeyCode(entry const &e) {

	if (e.first == "PositionerKeyToggle") {
		PositionerKeyToggle = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyDimensionX") {
		PositionerKeyDimensionX = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyDimensionY") {
		PositionerKeyDimensionY = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyDimensionZ") {
		PositionerKeyDimensionZ = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyOperationMove") {
		PositionerKeyOperationMove = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyOperationRotate") {
		PositionerKeyOperationRotate = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyAdd") {
		PositionerKeyAdd = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeySubtract") {
		PositionerKeySubtract = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyMarkForSave") {
		PositionerKeyMarkForSave = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyShowCurrent") {
		PositionerKeyShowCurrent = std::stoi(e.second);
	}
	else if (e.first == "PositionerKeyReset") {
		PositionerKeyReset = std::stoi(e.second);
	}
}
...
void HandleIniFiles() {
	...
	for (size_t i = 0; i < modSettingFiles.size(); i++)
	{
		...
		for (entry const &e : dict) {
			...
			else if (e.first.find("PositionerKey") != std::string::npos) {
				SetKeyCode(e);
			}
			else if (e.first.find("AdditionalSkeleton") != std::string::npos) {

				auto v = e.second;

				if (std::find(AdditonalSkeletonPaths.begin(), AdditonalSkeletonPaths.end(), v) == AdditonalSkeletonPaths.end()) {
					std::transform(v.begin(), v.end(), v.begin(), ::tolower);
					AdditonalSkeletonPaths.push_back(v);
				}
				//_MESSAGE("IsHolsterPositionSaveLogginEnabled");
			}
				...
				if (e.first.find("ExcludedNPCFormId") != std::string::npos) {
					...
					if (f) {
						...
						if (n) {
							...
							_MESSAGE(std::to_string(cc).c_str());
						}
					}
				}
			}
