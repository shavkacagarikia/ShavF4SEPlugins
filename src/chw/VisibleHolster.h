...
namespace VisibleHolster {
	...
	void handleMovingPartsTransformCorrection(NiAVObject* root, std::vector<const char *>* skinnedNodeNames = nullptr);

	std::string GetAttachNodeFromWeapon(TESObjectWEAP::InstanceData* instance, TESObjectWEAP* weap);

	void ClearAllHolsters(Actor* actor, bool removeAdditionalItems = false);
