...
bool InstWEAPHasKeyword(TESObjectWEAP::InstanceData* thisInstance, TESObjectWEAP* weap, BGSKeyword* kwdToCheck) {
	BGSKeywordForm * keywordForm = nullptr;

	keywordForm = thisInstance->keywords;
	if (!keywordForm) {
		bool r = HasKeyword(weap, kwdToCheck);
		return r;
	}
...
bool IsValidBSString(BSFixedString str) {
...
bool IsValidBSStringPtr(BSFixedString* str) {
	if (str && str->data && str->data->data) {
		return true;
	}
	return false;
